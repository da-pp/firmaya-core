 ...
 * ver ServicioRegistroAuditoria), evita inyeccion de formulas CSV y respeta los mismos
 * filtros que la consulta paginada.
 */
@Service
@Transactional(readOnly = true)
public class ServicioExportacionAuditoria {
    ...
    public ServicioExportacionAuditoria(RepositorioEventoAuditoria repositorioEventoAuditoria,
                                         ServicioConsultaAuditoria servicioConsultaAuditoria) {
    ...
    public byte[] exportar(ConsultaAuditoriaDto consulta) {
        ...
        var eventos = repositorioEventoAuditoria.buscar(
        ...
        try (PrintWriter escritor = new PrintWriter(salida, false, StandardCharsets.UTF_8)) {
            ...
            for (var dto : eventos.stream().map(servicioConsultaAuditoria::aResumenPublico).toList()) {
