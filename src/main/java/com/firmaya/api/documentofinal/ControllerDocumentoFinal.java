...
public class ControllerDocumentoFinal {

    private final ServiceDocumentoFinal serviceDocumentoFinal;

    public ControllerDocumentoFinal(ServiceDocumentoFinal serviceDocumentoFinal) {
        this.serviceDocumentoFinal = serviceDocumentoFinal;
    }
    ...
    public DocumentoFinalDto obtenerMetadatos(@PathVariable UUID idContrato, Authentication autenticacion) {
        ...
        if (principal instanceof ContextoUsuarioAutenticado interno) {
            return serviceDocumentoFinal.obtenerMetadatosParaUsuarioInterno(idContrato, interno.idUsuario());
        }
        if (principal instanceof ContextoParticipanteAutenticado externo) {
            return serviceDocumentoFinal.obtenerMetadatosParaParticipanteExterno(idContrato, externo);
        }
    ...
    public ResponseEntity<DocumentoFinalDto> generar(@PathVariable UUID idContrato,
        ...
        var resultado = serviceDocumentoFinal.generarIdempotentemente(idContrato, usuario.idUsuario());
    ...
    public ResponseEntity<byte[]> descargar(@PathVariable UUID idContrato, Authentication autenticacion) {
        ...
        if (principal instanceof ContextoUsuarioAutenticado interno) {
            archivo = serviceDocumentoFinal.descargarParaUsuarioInterno(idContrato, interno.idUsuario());
        } else if (principal instanceof ContextoParticipanteAutenticado externo) {
            archivo = serviceDocumentoFinal.descargarParaParticipanteExterno(idContrato, externo);
        } else {
            throw new AccesoDenegadoNegocioException("Sesion no reconocida.");
        }
