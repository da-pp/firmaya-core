...
public class ControllerComentarios {
    ...
    private final ServiceComentarios serviceComentarios;

    public ControllerComentarios(ServiceComentarios serviceComentarios) {
        this.serviceComentarios = serviceComentarios;
    }
    ...
    public List<ComentarioDto> listar(@PathVariable UUID idContrato, @PathVariable UUID idVersion,
        ...
        if (principal instanceof ContextoUsuarioAutenticado interno) {
            return serviceComentarios.listarPorVersionParaUsuarioInterno(idContrato, idVersion, interno.idUsuario());
        }
        if (principal instanceof ContextoParticipanteAutenticado externo) {
            return serviceComentarios.listarPorVersionParaParticipanteExterno(idContrato, idVersion, externo);
        }
    ...
    public ResponseEntity<ComentarioDto> publicar(@PathVariable UUID idContrato, @PathVariable UUID idVersion,
        ...
        if (principal instanceof ContextoUsuarioAutenticado interno) {
            comentario = serviceComentarios.publicarComoUsuarioInterno(idContrato, idVersion, interno.idUsuario(), solicitud);
        } else if (principal instanceof ContextoParticipanteAutenticado externo) {
            comentario = serviceComentarios.publicarComoParticipanteExterno(idContrato, idVersion, externo, solicitud);
        } else {
            throw new AccesoDenegadoNegocioException("Sesion no reconocida.");
        }
