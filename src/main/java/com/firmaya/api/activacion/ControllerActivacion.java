...
public class ControllerActivacion {

    private final ServiceActivacion serviceActivacion;

    public ControllerActivacion(ServiceActivacion serviceActivacion) {
        this.serviceActivacion = serviceActivacion;
    }

    @PostMapping("/validar")
    public ValidacionActivacionDto validarToken(@Valid @RequestBody SolicitudTokenActivacion solicitud) {
        return serviceActivacion.validarToken(solicitud.token());
    }

    @PostMapping("/completar")
    public ResultadoActivacionDto completarActivacion(@Valid @RequestBody SolicitudCompletarActivacion solicitud) {
        return serviceActivacion.completarActivacion(solicitud);
    }
}
