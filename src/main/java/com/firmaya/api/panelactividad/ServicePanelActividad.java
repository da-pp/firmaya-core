 ...
 * interpretacion tecnica provisional usada (ver Javadoc de cada metodo de RepositoryContrato).
...
public class ServicePanelActividad {
    ...
    public ServicePanelActividad(RepositoryContrato repositoryContrato,
    ...
    public PanelActividadDto obtenerResumen(LocalDate origen, LocalDate destino, List<EstadoContrato> estados) {
        ...
        long activos = repositoryContrato.contarActivos();
        long pendientesFirma = repositoryContrato.contarPendientesFirma();
        long firmadosEnPeriodo = repositoryContrato.contarFirmadosEnPeriodo(inicio, fin);
        long proximosAVencer = repositoryContrato.contarProximosAVencer(hoy, hoy.plusDays(DIAS_PROXIMO_VENCIMIENTO));

        List<ConteoPorEstadoDto> porEstado = repositoryContrato
        ...
        List<Contrato> recientes = repositoryContrato.findTop10ByOrderByFechaUltimaActividadDesc();
    ...
    private org.springframework.data.domain.Page<Contrato> buscarPorMetrica(MetricaPanel metrica, LocalDate[] rango,
        ...
        return switch (metrica) {
            case ACTIVOS -> repositoryContrato.buscarActivos(estados, pageable);
            case PENDIENTES_FIRMA -> repositoryContrato.buscarPendientesFirma(estados, pageable);
            case FIRMADOS_PERIODO -> repositoryContrato.buscarFirmadosEnPeriodo(
                    inicioDeDia(rango[0]), finDeDia(rango[1]), estados, pageable);
            case PROXIMOS_VENCER -> repositoryContrato.buscarProximosAVencer(
