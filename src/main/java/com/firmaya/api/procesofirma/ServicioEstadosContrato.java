 ...
 * cancelacion (EP-45, ServicioProcesoFirma) y FIRMADO es exclusivamente automatico (CU-08).
 */
@Service
@Transactional
public class ServicioEstadosContrato {
    ...
    public ServicioEstadosContrato(RepositorioContrato repositorioContrato,
    ...
    public OpcionesTransicionDto obtenerTransicionesManuales(UUID idContrato, UUID idUsuario) {
        ...
        boolean hayFirmantes = !repositorioParticipante
    ...
    private UUID congelarProcesoFirma(Contrato contrato, OffsetDateTime ahora) {
        List<Participante> firmantes = repositorioParticipante
        ...
        var versionActual = repositorioVersionContrato.findByIdAndContratoId(contrato.getIdVersionActual(), contrato.getId())
        ...
        proceso.activar();
        repositorioProcesoFirma.save(proceso);

        for (Participante firmante : firmantes) {
            repositorioFirmanteProceso.save(FirmanteProceso.crear(UUID.randomUUID(), proceso, firmante, ahora));
        }
