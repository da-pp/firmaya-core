...
public class ControladorAccesoExterno {

    private final ServicioAccesoExterno servicioAccesoExterno;

    public ControladorAccesoExterno(ServicioAccesoExterno servicioAccesoExterno) {
        this.servicioAccesoExterno = servicioAccesoExterno;
    }

    @PostMapping("/canjear")
    public ResponseEntity<SesionExternaDto> canjear(@Valid @RequestBody SolicitudCanjeAcceso solicitud,
                                                      HttpServletRequest request) {
        var resultado = servicioAccesoExterno.canjear(solicitud.token(), request.getRemoteAddr());
    ...
    public AccesoExternoDto obtenerContexto(@AuthenticationPrincipal ContextoParticipanteAutenticado contexto) {
        return servicioAccesoExterno.obtenerContexto(contexto);
    }
